<template>
  <div class="ele-body">
    <a-card :bordered="false">
      <vxe-grid ref='competitionGrid' v-bind="gridOptions" class="my-grid66">
        <!--搜索表单 -->
        <template #form>
          <a-form
            :model="where"
            ...
            <a-row>
              ...
              <a-col :lg="8" :md="8" :sm="12"
                     :xs="12">
                <a-form-item label="举办单位:" name="unitsId">
                  <m-entity-select
                    v-model:value="where.unitsId"
                    placeholder="请选择举办单位"
...
<!--            </a-button>-->
      ...
      <!--编辑弹窗-->
      <vxe-modal v-model="competitionListApp.editModalShowing" ref="competitionEditModal"
                 :title="gridOptions.showEditTitle"
                 id="competitionEditModal"
      ...
      >
        <template #default>
          <div id="competitionEditModalContent">
          </div>
        </template>
        <template #footer>
          <div id="competitionEditModalFooter">
          </div>
          <teleport to="#competitionEditModalContent">
            <competition-edit ref="competition-edit"/>
          </teleport>
        </template>
      </vxe-modal>

      <!--查看弹窗-->
      <vxe-modal v-model="competitionListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                 id="competitionViewModal"
                 width="600"
                 min-width="460"
                 min-height="320"
                 :destroy-on-close="true"
                 ...
                 show-footer show-zoom resize remember storage transfer dblclickZoom>
        <template #default>
          <competition-detail ref="competition-detail"/>
        </template>
      </vxe-modal>
    </a-card>
  </div>
...
import CompetitionEdit
  from "@/views/ams/competitionEntity/competition/competition-edit";
import CompetitionDetail
  from "@/views/ams/competitionEntity/competition/competition-detail";
import {
  PlusOutlined,
...
export default defineComponent({
  ...
  setup() {
    ...
    const competitionGrid = ref({})// as VxeGridInstance
    // 搜索表单是否展开
    ...
    const competitionListApp = reactive({
    ...
    const gridOptions = reactive({
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({page, sorts, filters}) => {  // 接收 Promise
            ...
            //
            const $grid = competitionGrid.value
            $grid.queryParams = queryParams
            return CompetitionService.findCompetitions(queryParams).catch(error => {
      ...
      exportConfig: {
        ...
        exportMethod({options}) {
          ...
          // 开始服务端导出
          return CompetitionService.findCompetitionsForExport(body).then(data => {
    ...
    provide('competitionListApp', competitionListApp)
    provide('gridOptions', gridOptions)

    /*TODO:新建事件*/
    const addEvent = () => {
      competitionListApp.editModalShowing = true
      competitionListApp.currentId = null;
      competitionListApp.addModalForEdit = true;
      gridOptions.showEditTitle = "新建";
    }

    /*TODO:新建独立页*/
    const addEvent2 = () => {
      router.push(`/ams/competitionEntity/competition/competition-add`);
    }
    /*TODO:编辑事件*/
    const editEvent = (row) => {
      competitionListApp.editModalShowing = true
      competitionListApp.currentId = row.eid;
      competitionListApp.editModalForEdit = true;
      gridOptions.showEditTitle = "编辑";
    }
    /*TODO:编辑独立页*/
    const editEvent2 = (eid) => {
      router.push(`/ams/competitionEntity/competition/competition-edit/${eid}`);
    }
    ...
    const viewEvent = (row) => {
      competitionListApp.currentId = row.eid;
      competitionListApp.viewModalShowing = true
      gridOptions.showDetailsTitle = '查看';
    }

    /*TODO:删除事件*/
    const deleteEvent = (row) => {
      CompetitionService.deleteCompetition(row.eid).then(() => {
