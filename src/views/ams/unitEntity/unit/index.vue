<template>
  <div class="ele-body">
    <a-card :bordered="false">
      <vxe-grid ref='unitGrid' v-bind="gridOptions" class="my-grid66">
        <!--搜索表单 -->
        <template #form>
          <a-form
            :model="where"
            ...
            <a-row>
              ...
              <a-col :lg="8" :md="12" :sm="24"
                     :xs="24">
                <a-form-item label="举办单位:" name="name">
                  <a-input
...
<!--            </a-button>-->
      ...
      <!--编辑弹窗-->
      <vxe-modal v-model="unitListApp.editModalShowing" ref="unitEditModal"
      ...
      >
        <template #default>
          <div id="unitEditModalContent">
          </div>
        </template>
        <template #footer>
          <div id="unitEditModalFooter">
          </div>
          <teleport to="#unitEditModalContent">
            <unit-edit ref="unit-edit"/>
          </teleport>
        </template>
      </vxe-modal>

      <!--查看弹窗-->
      <vxe-modal v-model="unitListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                 id="unitViewModal"
                 width="600"
                 min-width="460"
                 min-height="320"
...
import UnitEdit
  from "@/views/ams/unitEntity/unit/unit-edit";
import UnitDetail
  from "@/views/ams/unitEntity/unit/unit-detail";
import {
  PlusOutlined,
...
export default defineComponent({
  ...
  setup() {
    ...
    const unitGrid = ref({})// as VxeGridInstance
    // 搜索表单是否展开
    ...
    const gridOptions = reactive({
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({page, sorts, filters}) => {  // 接收 Promise
            ...
            //
            const $grid = unitGrid.value
            $grid.queryParams = queryParams
            return UnitService.findUnits(queryParams).catch(error => {
      ...
      exportConfig: {
        ...
        exportMethod({options}) {
          ...
          // 开始服务端导出
          return UnitService.findUnitsForExport(body).then(data => {
    ...
    provide('unitListApp', unitListApp)
    provide('gridOptions', gridOptions)

    /*TODO:新建事件*/
    const addEvent = () => {
      unitListApp.editModalShowing = true
      unitListApp.currentId = null;
      unitListApp.addModalForEdit = true;
      gridOptions.showEditTitle = "新建";
    }

    /*TODO:新建独立页*/
    const addEvent2 = () => {
      router.push(`/ams/unitEntity/unit/unit-add`);
    }
    /*TODO:编辑事件*/
    const editEvent = (row) => {
      unitListApp.editModalShowing = true
      unitListApp.currentId = row.eid;
      unitListApp.editModalForEdit = true;
      gridOptions.showEditTitle = "编辑";
    }
    /*TODO:编辑独立页*/
    const editEvent2 = (eid) => {
      router.push(`/ams/unitEntity/unit/unit-edit/${eid}`);
    }
    ...
    const viewEvent = (row) => {
      unitListApp.currentId = row.eid;
      unitListApp.viewModalShowing = true
      gridOptions.showDetailsTitle = '查看';
    }

    /*TODO:删除事件*/
    const deleteEvent = (row) => {
      UnitService.deleteUnit(row.eid).then(() => {
