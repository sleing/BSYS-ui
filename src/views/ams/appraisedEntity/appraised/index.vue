<template>
  <div class="ele-body">
    <a-card :bordered="false">
      <vxe-grid ref='appraisedGrid' v-bind="gridOptions" class="my-grid66">
        <!--搜索表单 -->
        <template #form>
          <a-form
            :model="where"
            ...
            <a-row>
            <a-span style="font-weight: bold;font-size: 20px">
              审核通过的评选信息如下：
            </a-span>
...
<!--        </template>-->
      ...
      <!--编辑弹窗-->
      <vxe-modal v-model="appraisedListApp.editModalShowing" ref="appraisedEditModal"
                 :title="gridOptions.showEditTitle"
                 id="appraisedEditModal"
      ...
      >
        <template #default>
          <div id="appraisedEditModalContent">
          </div>
        </template>
        <template #footer>
          <div id="appraisedEditModalFooter">
          </div>
          <teleport to="#appraisedEditModalContent">
            <appraised-edit ref="appraised-edit"/>
          </teleport>
        </template>
      </vxe-modal>

      <!--查看弹窗-->
      <vxe-modal v-model="appraisedListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                 id="appraisedViewModal"
                 width="600"
                 min-width="460"
                 min-height="320"
                 :destroy-on-close="true"
                 ...
                 show-footer show-zoom resize remember storage transfer dblclickZoom>
        <template #default>
          <appraised-detail ref="appraised-detail"/>
        </template>
      </vxe-modal>
    </a-card>
  </div>
...
import AppraisedEdit
  from "@/views/ams/appraisedEntity/appraised/appraised-edit";
import AppraisedDetail
  from "@/views/ams/appraisedEntity/appraised/appraised-detail";
...
export default defineComponent({
  ...
  setup() {
    ...
    const appraisedGrid = ref({})// as VxeGridInstance
    // 搜索表单是否展开
    ...
    const appraisedListApp = reactive({
    ...
    const gridOptions = reactive({
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({page, sorts, filters}) => {  // 接收 Promise
            ...
            //
            const $grid = appraisedGrid.value
            $grid.queryParams = queryParams
            return AppraisedService.findAppraiseds(queryParams).catch(error => {
      ...
      exportConfig: {
        ...
        exportMethod({options}) {
          ...
          // 开始服务端导出
          return AppraisedService.findAppraisedsForExport(body).then(data => {
    ...
    provide('appraisedListApp', appraisedListApp)
    provide('gridOptions', gridOptions)

    /*TODO:新建事件*/
    const addEvent = () => {
      appraisedListApp.editModalShowing = true
      appraisedListApp.currentId = null;
      appraisedListApp.addModalForEdit = true;
      gridOptions.showEditTitle = "新建";
    }

    /*TODO:新建独立页*/
    const addEvent2 = () => {
      router.push(`/ams/appraisedEntity/appraised/appraised-add`);
    }
    /*TODO:编辑事件*/
    const editEvent = (row) => {
      appraisedListApp.editModalShowing = true
      appraisedListApp.currentId = row.eid;
      appraisedListApp.editModalForEdit = true;
      gridOptions.showEditTitle = "编辑";
    }
    /*TODO:编辑独立页*/
    const editEvent2 = (eid) => {
      router.push(`/ams/appraisedEntity/appraised/appraised-edit/${eid}`);
    }
    ...
    const viewEvent = (row) => {
      appraisedListApp.currentId = row.eid;
      appraisedListApp.viewModalShowing = true
      gridOptions.showDetailsTitle = '查看';
    }

    /*TODO:删除事件*/
    const deleteEvent = (row) => {
      AppraisedService.deleteAppraised(row.eid).then(() => {
