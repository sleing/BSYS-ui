<template>
  <div class="ele-body">
    <a-card :bordered="false">
      <vxe-grid ref='appraisingTypeGrid' v-bind="gridOptions" class="my-grid66">
        <!--搜索表单 -->
        <template #form>
          <a-form
            :model="where"
            ...
            <a-row>
              ...
              <a-col :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="介绍:" name="introduction">
                  <a-input
                    v-model:value="where.introduction"
                    :maxlength="255"
                    placeholder="请输入介绍"
                    allow-clear
                    class="ele-fluid"
      ...
      <!--编辑弹窗-->
      <vxe-modal v-model="appraisingTypeListApp.editModalShowing" ref="appraisingTypeEditModal"
                 :title="gridOptions.showEditTitle"
                 id="appraisingTypeEditModal"
      ...
      >
        <template #default>
          <div id="appraisingTypeEditModalContent">
          </div>
        </template>
        <template #footer>
          <div id="appraisingTypeEditModalFooter">
          </div>
          <teleport to="#appraisingTypeEditModalContent">
            <appraisingType-edit ref="appraisingType-edit"/>
          </teleport>
        </template>
      </vxe-modal>

      <!--查看弹窗-->
      <vxe-modal v-model="appraisingTypeListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                 id="appraisingTypeViewModal"
                 width="600"
                 min-width="460"
                 min-height="320"
                 :destroy-on-close="true"
                 ...
                 show-footer show-zoom resize remember storage transfer dblclickZoom>
        <template #default>
          <appraisingType-detail ref="appraisingType-detail"/>
        </template>
      </vxe-modal>
    </a-card>
  </div>
...
import AppraisingTypeEdit
  from "@/views/ams/appraisingTypeEntity/appraisingType/appraisingType-edit";
import AppraisingTypeDetail
  from "@/views/ams/appraisingTypeEntity/appraisingType/appraisingType-detail";
import {
  PlusOutlined,
  DeleteOutlined,
  DownOutlined,
...
export default defineComponent({
  ...
  setup() {
    ...
    const appraisingTypeGrid = ref({})// as VxeGridInstance
    // 搜索表单是否展开
    ...
    const appraisingTypeListApp = reactive({
    ...
    const gridOptions = reactive({
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({page, sorts, filters}) => {  // 接收 Promise
            ...
            //
            const $grid = appraisingTypeGrid.value
            $grid.queryParams = queryParams
            return AppraisingTypeService.findAppraisingTypes(queryParams).catch(error => {
      ...
      exportConfig: {
        ...
        exportMethod({options}) {
          ...
          // 开始服务端导出
          return AppraisingTypeService.findAppraisingTypesForExport(body).then(data => {
    ...
    provide('appraisingTypeListApp', appraisingTypeListApp)
    provide('gridOptions', gridOptions)

    /*TODO:新建事件*/
    const addEvent = () => {
      appraisingTypeListApp.editModalShowing = true
      appraisingTypeListApp.currentId = null;
      appraisingTypeListApp.addModalForEdit = true;
      gridOptions.showEditTitle = "新建";
    }

    /*TODO:新建独立页*/
    const addEvent2 = () => {
      router.push(`/ams/appraisingTypeEntity/appraisingType/appraisingType-add`);
    }
    /*TODO:编辑事件*/
    const editEvent = (row) => {
      appraisingTypeListApp.editModalShowing = true
      appraisingTypeListApp.currentId = row.eid;
      appraisingTypeListApp.editModalForEdit = true;
      gridOptions.showEditTitle = "编辑";
    }
    /*TODO:编辑独立页*/
    const editEvent2 = (eid) => {
      router.push(`/ams/appraisingTypeEntity/appraisingType/appraisingType-edit/${eid}`);
    }
    ...
    const viewEvent = (row) => {
      appraisingTypeListApp.currentId = row.eid;
      appraisingTypeListApp.viewModalShowing = true
      gridOptions.showDetailsTitle = '查看';
    }

    /*TODO:删除事件*/
    const deleteEvent = (row) => {
      AppraisingTypeService.deleteAppraisingType(row.eid).then(() => {
