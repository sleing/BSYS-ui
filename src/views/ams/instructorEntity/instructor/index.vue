<template>
  <div class="ele-body">
    <a-card :bordered="false">
      <vxe-grid ref='instructorGrid' v-bind="gridOptions" class="my-grid66">
        <!--搜索表单 -->
        <template #form>
          <a-form
            :model="where"
      ...
      <!--编辑弹窗-->
      <vxe-modal v-model="instructorListApp.editModalShowing" ref="instructorEditModal"
      ...
      >
        <template #default>
          <div id="instructorEditModalContent">
          </div>
        </template>
        <template #footer>
          <div id="instructorEditModalFooter">
          </div>
          <teleport to="#instructorEditModalContent">
            <instructor-edit ref="instructor-edit"/>
          </teleport>
        </template>
      </vxe-modal>

      <!--查看弹窗-->
      <vxe-modal v-model="instructorListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                 id="instructorViewModal"
                 width="600"
                 min-width="460"
                 min-height="320"
                 :destroy-on-close="true"
                 ...
                 show-footer show-zoom resize remember storage transfer dblclickZoom>
        <template #default>
          <instructor-detail ref="instructor-detail"/>
        </template>
      </vxe-modal>
    </a-card>
  </div>
...
import InstructorEdit
  from "@/views/ams/instructorEntity/instructor/instructor-edit";
import InstructorDetail
  from "@/views/ams/instructorEntity/instructor/instructor-detail";
import {
  PlusOutlined,
  DeleteOutlined,
  DownOutlined,
...
export default defineComponent({
  ...
  setup() {
    ...
    const instructorGrid = ref({})// as VxeGridInstance
    // 搜索表单是否展开
    ...
    const instructorListApp = reactive({
    ...
    const gridOptions = reactive({
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({page, sorts, filters}) => {  // 接收 Promise
            ...
            //
            const $grid = instructorGrid.value
            $grid.queryParams = queryParams
            return InstructorService.findInstructors(queryParams).catch(error => {
      ...
      exportConfig: {
        ...
        exportMethod({options}) {
          ...
          // 开始服务端导出
          return InstructorService.findInstructorsForExport(body).then(data => {
    ...
    provide('instructorListApp', instructorListApp)
    provide('gridOptions', gridOptions)

    /*TODO:新建事件*/
    const addEvent = () => {
      instructorListApp.editModalShowing = true
      instructorListApp.currentId = null;
      instructorListApp.addModalForEdit = true;
      gridOptions.showEditTitle = "新建";
    }

    /*TODO:新建独立页*/
    const addEvent2 = () => {
      router.push(`/ams/instructorEntity/instructor/instructor-add`);
    }
    /*TODO:编辑事件*/
    const editEvent = (row) => {
      instructorListApp.editModalShowing = true
      instructorListApp.currentId = row.eid;
      instructorListApp.editModalForEdit = true;
      gridOptions.showEditTitle = "编辑";
    }
    /*TODO:编辑独立页*/
    const editEvent2 = (eid) => {
      router.push(`/ams/instructorEntity/instructor/instructor-edit/${eid}`);
    }
    ...
    const viewEvent = (row) => {
      instructorListApp.currentId = row.eid;
      instructorListApp.viewModalShowing = true
      gridOptions.showDetailsTitle = '查看';
    }

    /*TODO:删除事件*/
    const deleteEvent = (row) => {
      InstructorService.deleteInstructor(row.eid).then(() => {
