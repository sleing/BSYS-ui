<template>
  <div class="ele-body">
    <a-card :bordered="false">
      <vxe-grid ref='studentInfoGrid' v-bind="gridOptions" class="my-grid66">
        <!--搜索表单 -->
        <template #form>
          <a-form
            :model="where"
...
<!--            </a-button>-->
      ...
      <!--查看弹窗-->
      <vxe-modal v-model="studentInfoListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                 id="studentInfoViewModal"
                 width="600"
                 min-width="460"
                 min-height="320"
                 :destroy-on-close="true"
                 ...
                 show-footer show-zoom resize remember storage transfer dblclickZoom>
        <template #default>
          <studentInfo-detail ref="studentInfo-detail"/>
        </template>
      </vxe-modal>
    </a-card>
  </div>
...
// import {StudentInfoervice} from "@/views/ams/studentEntityEntity/studentInfo/studentInfoService";
import {StudentInfoService} from "@/views/ams/studentEntity/studentInfo/studentInfoService";
import StudentInfoEdit
  from "@/views/ams/studentEntity/studentInfo/studentInfo-edit";
import StudentInfoDetail
  from "@/views/ams/studentEntity/studentInfo/studentInfo-detail";
import {
  PlusOutlined,
  DownOutlined,
  UpOutlined,
...
export default defineComponent({
  ...
  setup() {
    ...
    const studentInfoGrid = ref({})// as VxeGridInstance
    // 搜索表单是否展开
    ...
    const gridOptions = reactive({
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({page, sorts, filters}) => {  // 接收 Promise
            ...
            //
            const $grid = studentInfoGrid.value
            $grid.queryParams = queryParams
            return StudentInfoService.findStudentInfos(queryParams).catch(error => {
      ...
      exportConfig: {
        ...
        exportMethod({options}) {
          ...
          // 开始服务端导出
          return StudentInfoService.findStudentInfosForExport(body).then(data => {
    ...
    provide('studentInfoListApp', studentInfoListApp)
    provide('gridOptions', gridOptions)

    /*TODO:新建事件*/
    const addEvent = () => {
      studentInfoListApp.editModalShowing = true
      studentInfoListApp.currentId = null;
      studentInfoListApp.addModalForEdit = true;
      gridOptions.showEditTitle = "新建";
    }

    /*TODO:新建独立页*/
    const addEvent2 = () => {
      router.push(`/ams/studentEntity/studentInfo/studentInfo-add`);
    }
    /*TODO:编辑事件*/
    const editEvent = (row) => {
      studentInfoListApp.editModalShowing = true
      studentInfoListApp.currentId = row.eid;
      studentInfoListApp.editModalForEdit = true;
      gridOptions.showEditTitle = "编辑";
    }
    /*TODO:编辑独立页*/
    const editEvent2 = (eid) => {
      router.push(`/ams/studentEntity/studentInfo/studentInfo-edit/${eid}`);
    }
    ...
    const viewEvent = (row) => {
      studentInfoListApp.currentId = row.eid;
      studentInfoListApp.viewModalShowing = true
      gridOptions.showDetailsTitle = '查看';
    }

    /*TODO:删除事件*/
    const deleteEvent = (row) => {
      StudentInfoService.deleteStudentInfo(row.eid).then(() => {
