<template>
  <div class="ele-body">
    <a-card :bordered="false">
      <vxe-grid ref='collegeGrid' v-bind="gridOptions" class="my-grid66">
        <!--搜索表单 -->
        <template #form>
          <a-form
            :model="where"
      ...
      <!--编辑弹窗-->
      <vxe-modal v-model="collegeListApp.editModalShowing" ref="collegeEditModal"
      ...
      >
        <template #default>
          <div id="collegeEditModalContent">
          </div>
        </template>
        <template #footer>
          <div id="collegeEditModalFooter">
          </div>
          <teleport to="#collegeEditModalContent">
            <college-edit ref="college-edit"/>
          </teleport>
        </template>
      </vxe-modal>

      <!--查看弹窗-->
      <vxe-modal v-model="collegeListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                 id="collegeViewModal"
                 width="600"
                 min-width="460"
                 min-height="320"
                 ...
                 show-footer show-zoom resize remember storage transfer dblclickZoom>
        <template #default>
          <college-detail ref="college-detail"/>
        </template>
      </vxe-modal>
    </a-card>
  </div>
...
import CollegeEdit
  from "@/views/ams/collegeEntity/college/college-edit";
import CollegeDetail
  from "@/views/ams/collegeEntity/college/college-detail";
import {
  PlusOutlined,
  DeleteOutlined,
  DownOutlined,
...
export default defineComponent({
  ...
  setup() {
    ...
    const collegeGrid = ref({})// as VxeGridInstance
    // 搜索表单是否展开
    ...
    const gridOptions = reactive({
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({page, sorts, filters}) => {  // 接收 Promise
            ...
            //
            const $grid = collegeGrid.value
            $grid.queryParams = queryParams
            return CollegeService.findColleges(queryParams).catch(error => {
      ...
      exportConfig: {
        ...
        exportMethod({options}) {
          ...
          // 开始服务端导出
          return CollegeService.findCollegesForExport(body).then(data => {
    ...
    provide('collegeListApp', collegeListApp)
    provide('gridOptions', gridOptions)

    /*TODO:新建事件*/
    const addEvent = () => {
      collegeListApp.editModalShowing = true
      collegeListApp.currentId = null;
      collegeListApp.addModalForEdit = true;
      gridOptions.showEditTitle = "新建";
    }

    /*TODO:新建独立页*/
    const addEvent2 = () => {
      router.push(`/ams/collegeEntity/college/college-add`);
    }
    /*TODO:编辑事件*/
    const editEvent = (row) => {
      collegeListApp.editModalShowing = true
      collegeListApp.currentId = row.eid;
      collegeListApp.editModalForEdit = true;
      gridOptions.showEditTitle = "编辑";
    }
    /*TODO:编辑独立页*/
    const editEvent2 = (eid) => {
      router.push(`/ams/collegeEntity/college/college-edit/${eid}`);
    }
    ...
    const viewEvent = (row) => {
      collegeListApp.currentId = row.eid;
      collegeListApp.viewModalShowing = true
      gridOptions.showDetailsTitle = '查看';
    }

    /*TODO:删除事件*/
    const deleteEvent = (row) => {
      CollegeService.deleteCollege(row.eid).then(() => {
