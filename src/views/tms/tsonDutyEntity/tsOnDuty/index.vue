<template>
  <div class="ele-body">
    <a-card :bordered="false">
      <vxe-grid ref='tsOnDutyGrid' v-bind="gridOptions" class="my-grid66">
        <!--搜索表单 -->
        <template #form>
          <a-form
            :model="where"
            ...
            <a-row>
              ...
              <a-col :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="值班时间:" name="onDutyTime">
                  <a-date-picker
                    v-model:value="where.onDutyTime"
                    placeholder="请选择值班时间"
              ...
              <a-col :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="值班地点:" name="onDutyAddressId">
                  <m-entity-select
                    v-model:value="where.onDutyAddressId"
                    placeholder="请选择值班地点"
                    module="tsonDutyEntity"
                    entity="TsWorkPlace"
                    class="ele-fluid"
                  />
                </a-form-item>
              ...
              <a-col :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="值班人员:" name="onDutyStuId">
                  <m-entity-select
                    v-model:value="where.onDutyStuId"
                    placeholder="请选择值班人员"
                    module="tsonDutyEntity"
      ...
      <!--编辑弹窗-->
      <vxe-modal v-model="tsOnDutyListApp.editModalShowing" ref="tsOnDutyEditModal"
                 :title="gridOptions.showEditTitle"
                 id="tsOnDutyEditModal"
      ...
      >
        <template #default>
          <div id="tsOnDutyEditModalContent">
          </div>
        </template>
        <template #footer>
          <div id="tsOnDutyEditModalFooter">
          </div>
          <teleport to="#tsOnDutyEditModalContent">
            <tsOnDuty-edit ref="tsOnDuty-edit"/>
          </teleport>
        </template>
      </vxe-modal>

      <!--查看弹窗-->
      <vxe-modal v-model="tsOnDutyListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                 id="tsOnDutyViewModal"
                 width="600"
                 min-width="460"
                 min-height="320"
                 :destroy-on-close="true"
                 ...
                 show-footer show-zoom resize remember storage transfer dblclickZoom>
        <template #default>
          <tsOnDuty-detail ref="tsOnDuty-detail"/>
        </template>
      </vxe-modal>
    </a-card>
  </div>
...
import TsOnDutyEdit
  from "@/views/tms/tsonDutyEntity/tsOnDuty/tsOnDuty-edit";
import TsOnDutyDetail
  from "@/views/tms/tsonDutyEntity/tsOnDuty/tsOnDuty-detail";
import {
  PlusOutlined,
  DeleteOutlined,
  DownOutlined,
...
export default defineComponent({
  ...
  setup() {
    ...
    const tsOnDutyGrid = ref({})// as VxeGridInstance
    // 搜索表单是否展开
    ...
    const tsOnDutyListApp = reactive({
    ...
    const gridOptions = reactive({
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({page, sorts, filters}) => {  // 接收 Promise
            ...
            //
            const $grid = tsOnDutyGrid.value
            $grid.queryParams = queryParams
            return TsOnDutyService.findTsOnDutys(queryParams).catch(error => {
      ...
      exportConfig: {
        ...
        exportMethod({options}) {
          ...
          // 开始服务端导出
          return TsOnDutyService.findTsOnDutysForExport(body).then(data => {
    ...
    provide('tsOnDutyListApp', tsOnDutyListApp)
    provide('gridOptions', gridOptions)

    /*TODO:新建事件*/
    const addEvent = () => {
      tsOnDutyListApp.editModalShowing = true
      tsOnDutyListApp.currentId = null;
      tsOnDutyListApp.addModalForEdit = true;
      gridOptions.showEditTitle = "新建";
    }

    /*TODO:新建独立页*/
    const addEvent2 = () => {
      router.push(`/tms/tsonDutyEntity/tsOnDuty/tsOnDuty-add`);
    }
    /*TODO:编辑事件*/
    const editEvent = (row) => {
      tsOnDutyListApp.editModalShowing = true
      tsOnDutyListApp.currentId = row.eid;
      tsOnDutyListApp.editModalForEdit = true;
      gridOptions.showEditTitle = "编辑";
    }
    /*TODO:编辑独立页*/
    const editEvent2 = (eid) => {
      router.push(`/tms/tsonDutyEntity/tsOnDuty/tsOnDuty-edit/${eid}`);
    }
    ...
    const viewEvent = (row) => {
      tsOnDutyListApp.currentId = row.eid;
      tsOnDutyListApp.viewModalShowing = true
      gridOptions.showDetailsTitle = '查看';
    }

    /*TODO:删除事件*/
    const deleteEvent = (row) => {
      TsOnDutyService.deleteTsOnDuty(row.eid).then(() => {
