<template>
  ...
  <div class="ele-body">
    <a-card :bordered="false">
            ...
            <a-row>>
              <a-col :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="部门级别:" name="departmentLevel">
                  <m-dict-select
                    v-model:value="where.departmentLevel"
                    placeholder="请选择部门级别"
                    dict="部门级别"
                    class="ele-fluid"
                  />
              ...
              <a-col :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="部门主管老师:" name="supervisorTeaId">
                  <m-entity-select
                    v-model:value="where.supervisorTeaId"
              ...
              <a-col :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="部门主管学生:" name="supervisorStuId">
                  <m-entity-select
                    v-model:value="where.supervisorStuId"
              ...
              <a-col v-if="searchExpand" :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="部门负责人:" name="viceLeader">
                  <a-input
                    v-model:value="where.viceLeader"
                    :maxlength="255"
              ...
              <a-col v-if="searchExpand" :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="部门描述:" name="description">
                  <a-input
                    v-model:value="where.description"
                    :maxlength="255"
                    placeholder="请输入部门描述"
                    allow-clear
                    class="ele-fluid"
      ...
      <!--编辑弹窗-->
      <vxe-modal v-model="tsDepartmentListApp.editModalShowing" ref="tsDepartmentEditModal"
                 :title="gridOptions.showEditTitle"
                 id="tsDepartmentEditModal"
      ...
      >
        <template #default>
          <div id="tsDepartmentEditModalContent">
          </div>
        </template>
        <template #footer>
          <div id="tsDepartmentEditModalFooter">
          </div>
          <teleport to="#tsDepartmentEditModalContent">
            <tsDepartment-edit ref="tsDepartment-edit"/>
          </teleport>
        </template>
      </vxe-modal>

      <!--查看弹窗-->
      <vxe-modal v-model="tsDepartmentListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                 id="tsDepartmentViewModal"
                 width="600"
                 min-width="460"
                 min-height="320"
                 :destroy-on-close="true"
                 ...
                 show-footer show-zoom resize remember storage transfer dblclickZoom>
        <template #default>
          <tsDepartment-detail ref="tsDepartment-detail"/>
        </template>
      </vxe-modal>
    </a-card>
  </div>
...
import TsDepartmentEdit
  from "@/views/tms/tsdepartmentEntity/tsDepartment/tsDepartment-edit";
import TsDepartmentDetail
  from "@/views/tms/tsdepartmentEntity/tsDepartment/tsDepartment-detail";
import {
  PlusOutlined,
  DeleteOutlined,
  DownOutlined,
...
export default defineComponent({
  ...
  setup() {
    ...
    const tsDepartmentGrid = ref({})// as VxeGridInstance
    // 搜索表单是否展开
    ...
    const tsDepartmentListApp = reactive({
    ...
    const gridOptions = reactive({
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({page, sorts, filters}) => {  // 接收 Promise
            ...
            //
            const $grid = tsDepartmentGrid.value
            $grid.queryParams = queryParams
            return TsDepartmentService.findTsDepartments(queryParams).catch(error => {
      ...
      columns: [
        ...
        {
          field: 'supervisorTeaName',
          title: '部门主管老师',
          sortable: true
        },
        {
          field: 'supervisorStuName',
          title: '部门主管学生',
          sortable: true
        },
        {
      ...
      exportConfig: {
        ...
        exportMethod({options}) {
          ...
          // 开始服务端导出
          return TsDepartmentService.findTsDepartmentsForExport(body).then(data => {
    ...
    provide('tsDepartmentListApp', tsDepartmentListApp)
    provide('gridOptions', gridOptions)

    /*TODO:新建事件*/
    const addEvent = () => {
      tsDepartmentListApp.editModalShowing = true
      tsDepartmentListApp.currentId = null;
      tsDepartmentListApp.addModalForEdit = true;
      gridOptions.showEditTitle = "新建";
    }

    /*TODO:新建独立页*/
    const addEvent2 = () => {
      router.push(`/tms/tsdepartmentEntity/tsDepartment/tsDepartment-add`);
    }
    /*TODO:编辑事件*/
    const editEvent = (row) => {
      tsDepartmentListApp.editModalShowing = true
      tsDepartmentListApp.currentId = row.eid;
      tsDepartmentListApp.editModalForEdit = true;
      gridOptions.showEditTitle = "编辑";
    }
    /*TODO:编辑独立页*/
    const editEvent2 = (eid) => {
      router.push(`/tms/tsdepartmentEntity/tsDepartment/tsDepartment-edit/${eid}`);
    }
    ...
    const viewEvent = (row) => {
      tsDepartmentListApp.currentId = row.eid;
      tsDepartmentListApp.viewModalShowing = true
      gridOptions.showDetailsTitle = '查看';
    }

    /*TODO:删除事件*/
    const deleteEvent = (row) => {
      TsDepartmentService.deleteTsDepartment(row.eid).then(() => {
