<template>
    <div class="ele-body">
        <a-card :bordered="false">
            <vxe-grid ref='tsWorkPlaceGrid' v-bind="gridOptions" class="my-grid66">
                <!--搜索表单 -->
                <template #form>
                    <a-form
                            :model="where"
                        ...
                        <a-row>
                                        ...
                                        <a-col  :lg="6" :md="12" :sm="24"
                                                             :xs="24">
                                            <a-form-item label="办公地点:" name="address">
                                                                                                    <a-input
            ...
            <!--编辑弹窗-->
            <vxe-modal v-model="tsWorkPlaceListApp.editModalShowing" ref="tsWorkPlaceEditModal"
                       :title="gridOptions.showEditTitle"
                       id="tsWorkPlaceEditModal"
            ...
            >
                <template #default>
                    <div id="tsWorkPlaceEditModalContent">
                    </div>
                </template>
                <template #footer>
                    <div id="tsWorkPlaceEditModalFooter">
                    </div>
                    <teleport to="#tsWorkPlaceEditModalContent">
                        <tsWorkPlace-edit ref="tsWorkPlace-edit"/>
                    </teleport>
                </template>
            </vxe-modal>

            <!--查看弹窗-->
            <vxe-modal v-model="tsWorkPlaceListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                       id="tsWorkPlaceViewModal"
                       width="600"
                       min-width="460"
                       min-height="320"
                       :destroy-on-close="true"
                       ...
                       show-footer show-zoom resize remember storage transfer dblclickZoom>
                <template #default>
                    <tsWorkPlace-detail ref="tsWorkPlace-detail"/>
                </template>
            </vxe-modal>
        </a-card>
    </div>
...
<script>
    ...
    import TsWorkPlaceEdit
        from "@/views/tms/tsworklaceEntity/tsWorkPlace/tsWorkPlace-edit";
    import TsWorkPlaceDetail
        from "@/views/tms/tsworklaceEntity/tsWorkPlace/tsWorkPlace-detail";
    import {
        PlusOutlined,
        DeleteOutlined,
        DownOutlined,
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const tsWorkPlaceGrid = ref({})// as VxeGridInstance
            // 搜索表单是否展开
            ...
            const tsWorkPlaceListApp = reactive({
            ...
            const gridOptions = reactive({
                ...
                proxyConfig: {
                    ...
                    ajax: {
                        ...
                        query: ({page, sorts, filters}) => {  // 接收 Promise
                            ...
                            //
                            const $grid = tsWorkPlaceGrid.value
                                $grid.queryParams = queryParams
                            return TsWorkPlaceService.findTsWorkPlaces(queryParams).catch(error => {
                ...
                exportConfig: {
                    ...
                    exportMethod({options}) {
                        ...
                        // 开始服务端导出
                        return TsWorkPlaceService.findTsWorkPlacesForExport(body).then(data => {
            ...
            provide('tsWorkPlaceListApp', tsWorkPlaceListApp)
            provide('gridOptions', gridOptions)

            /*TODO:新建事件*/
            const addEvent = () => {
                    tsWorkPlaceListApp.editModalShowing = true
                    tsWorkPlaceListApp.currentId = null;
                    tsWorkPlaceListApp.addModalForEdit = true;
                gridOptions.showEditTitle = "新建";
            }

            /*TODO:新建独立页*/
            const addEvent2 = () => {
                router.push(`/tms/tsworklaceEntity/tsWorkPlace/tsWorkPlace-add`);
            }
            /*TODO:编辑事件*/
            const editEvent = (row) => {
                    tsWorkPlaceListApp.editModalShowing = true
                    tsWorkPlaceListApp.currentId = row.eid;
                    tsWorkPlaceListApp.editModalForEdit = true;
                gridOptions.showEditTitle = "编辑";
            }
            /*TODO:编辑独立页*/
            const editEvent2 = (eid) => {
                router.push(`/tms/tsworklaceEntity/tsWorkPlace/tsWorkPlace-edit/${eid}`);
            }
            ...
            const viewEvent = (row) => {
                    tsWorkPlaceListApp.currentId = row.eid;
                    tsWorkPlaceListApp.viewModalShowing = true
                gridOptions.showDetailsTitle = '查看';
            }

            /*TODO:删除事件*/
            const deleteEvent = (row) => {
                    TsWorkPlaceService.deleteTsWorkPlace(row.eid).then(() => {
