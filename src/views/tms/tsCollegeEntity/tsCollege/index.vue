<template>
    <div class="ele-body">
        <a-card :bordered="false">
            <vxe-grid ref='tsCollegeGrid' v-bind="gridOptions" class="my-grid66">
                <!--搜索表单 -->
                <template #form>
                    <a-form
                            :model="where"
            ...
            <!--编辑弹窗-->
            <vxe-modal v-model="tsCollegeListApp.editModalShowing" ref="tsCollegeEditModal"
                       :title="gridOptions.showEditTitle"
                       id="tsCollegeEditModal"
            ...
            >
                <template #default>
                    <div id="tsCollegeEditModalContent">
                    </div>
                </template>
                <template #footer>
                    <div id="tsCollegeEditModalFooter">
                    </div>
                    <teleport to="#tsCollegeEditModalContent">
                        <tsCollege-edit ref="tsCollege-edit"/>
                    </teleport>
                </template>
            </vxe-modal>

            <!--查看弹窗-->
            <vxe-modal v-model="tsCollegeListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                       id="tsCollegeViewModal"
                       width="600"
                       min-width="460"
                       min-height="320"
                       :destroy-on-close="true"
                       ...
                       show-footer show-zoom resize remember storage transfer dblclickZoom>
                <template #default>
                    <tsCollege-detail ref="tsCollege-detail"/>
                </template>
            </vxe-modal>
        </a-card>
    </div>
...
<script>
    ...
    import TsCollegeEdit
        from "@/views/tms/tsCollegeEntity/tsCollege/tsCollege-edit";
    import TsCollegeDetail
        from "@/views/tms/tsCollegeEntity/tsCollege/tsCollege-detail";
    import {
        PlusOutlined,
        DeleteOutlined,
        DownOutlined,
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const tsCollegeGrid = ref({})// as VxeGridInstance
            // 搜索表单是否展开
            ...
            const tsCollegeListApp = reactive({
            ...
            const gridOptions = reactive({
                ...
                proxyConfig: {
                    ...
                    ajax: {
                        ...
                        query: ({page, sorts, filters}) => {  // 接收 Promise
                            ...
                            //
                            const $grid = tsCollegeGrid.value
                                $grid.queryParams = queryParams
                            return TsCollegeService.findTsColleges(queryParams).catch(error => {
                ...
                exportConfig: {
                    ...
                    exportMethod({options}) {
                        ...
                        // 开始服务端导出
                        return TsCollegeService.findTsCollegesForExport(body).then(data => {
            ...
            provide('tsCollegeListApp', tsCollegeListApp)
            provide('gridOptions', gridOptions)

            /*TODO:新建事件*/
            const addEvent = () => {
                    tsCollegeListApp.editModalShowing = true
                    tsCollegeListApp.currentId = null;
                    tsCollegeListApp.addModalForEdit = true;
                gridOptions.showEditTitle = "新建";
            }

            /*TODO:新建独立页*/
            const addEvent2 = () => {
                router.push(`/tms/tsCollegeEntity/tsCollege/tsCollege-add`);
            }
            /*TODO:编辑事件*/
            const editEvent = (row) => {
                    tsCollegeListApp.editModalShowing = true
                    tsCollegeListApp.currentId = row.eid;
                    tsCollegeListApp.editModalForEdit = true;
                gridOptions.showEditTitle = "编辑";
            }
            /*TODO:编辑独立页*/
            const editEvent2 = (eid) => {
                router.push(`/tms/tsCollegeEntity/tsCollege/tsCollege-edit/${eid}`);
            }
            ...
            const viewEvent = (row) => {
                    tsCollegeListApp.currentId = row.eid;
                    tsCollegeListApp.viewModalShowing = true
                gridOptions.showDetailsTitle = '查看';
            }

            /*TODO:删除事件*/
            const deleteEvent = (row) => {
                    TsCollegeService.deleteTsCollege(row.eid).then(() => {
