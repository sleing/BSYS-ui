<template>
    <div class="ele-body">
        <a-card :bordered="false">
            <vxe-grid ref='tsPositionGrid' v-bind="gridOptions" class="my-grid66">
                <!--搜索表单 -->
                <template #form>
                    <a-form
                            :model="where"
                        ...
                        <a-row>
                                        ...
                                        <a-col  :lg="6" :md="12" :sm="24"
                                            ...
                                            <a-form-item label="职务描述:" name="positionDetail">
                                                                                                    <a-input
                                                            v-model:value="where.positionDetail"
                                                            :maxlength="255"
                                                            placeholder="请输入职务描述"
                                                            allow-clear
                                                            class="ele-fluid"
            ...
            <!--编辑弹窗-->
            <vxe-modal v-model="tsPositionListApp.editModalShowing" ref="tsPositionEditModal"
                       :title="gridOptions.showEditTitle"
                       id="tsPositionEditModal"
            ...
            >
                <template #default>
                    <div id="tsPositionEditModalContent">
                    </div>
                </template>
                <template #footer>
                    <div id="tsPositionEditModalFooter">
                    </div>
                    <teleport to="#tsPositionEditModalContent">
                        <tsPosition-edit ref="tsPosition-edit"/>
                    </teleport>
                </template>
            </vxe-modal>

            <!--查看弹窗-->
            <vxe-modal v-model="tsPositionListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                       id="tsPositionViewModal"
                       width="600"
                       min-width="460"
                       min-height="320"
                       :destroy-on-close="true"
                       ...
                       show-footer show-zoom resize remember storage transfer dblclickZoom>
                <template #default>
                    <tsPosition-detail ref="tsPosition-detail"/>
                </template>
            </vxe-modal>
        </a-card>
    </div>
...
<script>
    ...
    import TsPositionEdit
        from "@/views/tms/tspositionEntity/tsPosition/tsPosition-edit";
    import TsPositionDetail
        from "@/views/tms/tspositionEntity/tsPosition/tsPosition-detail";
    import {
        PlusOutlined,
        DeleteOutlined,
        DownOutlined,
    ...
    export default defineComponent({
        ...
        setup() {
            ...
            const tsPositionGrid = ref({})// as VxeGridInstance
            // 搜索表单是否展开
            ...
            const tsPositionListApp = reactive({
            ...
            const gridOptions = reactive({
                ...
                proxyConfig: {
                    ...
                    ajax: {
                        ...
                        query: ({page, sorts, filters}) => {  // 接收 Promise
                            ...
                            //
                            const $grid = tsPositionGrid.value
                                $grid.queryParams = queryParams
                            return TsPositionService.findTsPositions(queryParams).catch(error => {
                ...
                exportConfig: {
                    ...
                    exportMethod({options}) {
                        ...
                        // 开始服务端导出
                        return TsPositionService.findTsPositionsForExport(body).then(data => {
            ...
            provide('tsPositionListApp', tsPositionListApp)
            provide('gridOptions', gridOptions)

            /*TODO:新建事件*/
            const addEvent = () => {
                    tsPositionListApp.editModalShowing = true
                    tsPositionListApp.currentId = null;
                    tsPositionListApp.addModalForEdit = true;
                gridOptions.showEditTitle = "新建";
            }

            /*TODO:新建独立页*/
            const addEvent2 = () => {
                router.push(`/tms/tspositionEntity/tsPosition/tsPosition-add`);
            }
            /*TODO:编辑事件*/
            const editEvent = (row) => {
                    tsPositionListApp.editModalShowing = true
                    tsPositionListApp.currentId = row.eid;
                    tsPositionListApp.editModalForEdit = true;
                gridOptions.showEditTitle = "编辑";
            }
            /*TODO:编辑独立页*/
            const editEvent2 = (eid) => {
                router.push(`/tms/tspositionEntity/tsPosition/tsPosition-edit/${eid}`);
            }
            ...
            const viewEvent = (row) => {
                    tsPositionListApp.currentId = row.eid;
                    tsPositionListApp.viewModalShowing = true
                gridOptions.showDetailsTitle = '查看';
            }

            /*TODO:删除事件*/
            const deleteEvent = (row) => {
                    TsPositionService.deleteTsPosition(row.eid).then(() => {
