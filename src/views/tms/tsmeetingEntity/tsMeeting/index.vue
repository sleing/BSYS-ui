<template>
  <div class="ele-body">
    <a-card :bordered="false">
      <vxe-grid ref='tsMeetingGrid' v-bind="gridOptions" class="my-grid66">
        <!--搜索表单 -->
        <template #form>
          <a-form
            :model="where"
            ...
            <a-row>
              ...
              <a-col :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="协办单位:" name="viceOrganizationId">
                  <m-entity-select
                    v-model:value="where.viceOrganizationId"
                    placeholder="请选择协办单位"
                    module="tsmeetingEntity"
                    entity="TsDepartment"
                    class="ele-fluid"
              ...
              <a-col v-if="searchExpand" :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="会议时间:" name="meetingDate">
                  <a-date-picker
                    v-model:value="where.meetingDate"
                    placeholder="请选择会议时间"
              ...
              <a-col v-if="searchExpand" :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="会议记录:" name="meetingRecord">
                  <a-input
                    v-model:value="where.meetingRecord"
                    :maxlength="255"
                    placeholder="请输入会议记录"
                    allow-clear
                    class="ele-fluid"
              ...
              <a-col v-if="searchExpand" :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="会议方式:" name="meetingMode">
                  <m-dict-select
                    v-model:value="where.meetingMode"
                    placeholder="请选择会议方式"
                    dict="会议方式"
                    class="ele-fluid"
                  />
              ...
              <a-col v-if="searchExpand" :lg="6" :md="12" :sm="24"
                ...
                <a-form-item label="会议地址:" name="meetingAddress">
                  <a-input
                    v-model:value="where.meetingAddress"
                    :maxlength="255"
                    placeholder="请输入会议地址"
                    allow-clear
                    class="ele-fluid"
      ...
      <!--编辑弹窗-->
      <vxe-modal v-model="tsMeetingListApp.editModalShowing" ref="tsMeetingEditModal"
                 :title="gridOptions.showEditTitle"
                 id="tsMeetingEditModal"
      ...
      >
        <template #default>
          <div id="tsMeetingEditModalContent">
          </div>
        </template>
        <template #footer>
          <div id="tsMeetingEditModalFooter">
          </div>
          <teleport to="#tsMeetingEditModalContent">
            <tsMeeting-edit ref="tsMeeting-edit"/>
          </teleport>
        </template>
      </vxe-modal>

      <!--查看弹窗-->
      <vxe-modal v-model="tsMeetingListApp.viewModalShowing" :title="gridOptions.showDetailsTitle"
                 id="tsMeetingViewModal"
                 width="600"
                 min-width="460"
                 min-height="320"
                 :destroy-on-close="true"
                 ...
                 show-footer show-zoom resize remember storage transfer dblclickZoom>
        <template #default>
          <tsMeeting-detail ref="tsMeeting-detail"/>
        </template>
      </vxe-modal>
    </a-card>
  </div>
...
import TsMeetingEdit
  from "@/views/tms/tsmeetingEntity/tsMeeting/tsMeeting-edit";
import TsMeetingDetail
  from "@/views/tms/tsmeetingEntity/tsMeeting/tsMeeting-detail";
import {
  PlusOutlined,
  DeleteOutlined,
  DownOutlined,
...
export default defineComponent({
  ...
  setup() {
    ...
    const tsMeetingGrid = ref({})// as VxeGridInstance
    // 搜索表单是否展开
    ...
    const tsMeetingListApp = reactive({
    ...
    const gridOptions = reactive({
      ...
      proxyConfig: {
        ...
        ajax: {
          ...
          query: ({page, sorts, filters}) => {  // 接收 Promise
            ...
            //
            const $grid = tsMeetingGrid.value
            $grid.queryParams = queryParams
            return TsMeetingService.findTsMeetings(queryParams).catch(error => {
      ...
      exportConfig: {
        ...
        exportMethod({options}) {
          ...
          // 开始服务端导出
          return TsMeetingService.findTsMeetingsForExport(body).then(data => {
    ...
    provide('tsMeetingListApp', tsMeetingListApp)
    provide('gridOptions', gridOptions)

    /*TODO:新建事件*/
    const addEvent = () => {
      tsMeetingListApp.editModalShowing = true
      tsMeetingListApp.currentId = null;
      tsMeetingListApp.addModalForEdit = true;
      gridOptions.showEditTitle = "新建";
    }

    /*TODO:新建独立页*/
    const addEvent2 = () => {
      router.push(`/tms/tsmeetingEntity/tsMeeting/tsMeeting-add`);
    }
    /*TODO:编辑事件*/
    const editEvent = (row) => {
      tsMeetingListApp.editModalShowing = true
      tsMeetingListApp.currentId = row.eid;
      tsMeetingListApp.editModalForEdit = true;
      gridOptions.showEditTitle = "编辑";
    }
    /*TODO:编辑独立页*/
    const editEvent2 = (eid) => {
      router.push(`/tms/tsmeetingEntity/tsMeeting/tsMeeting-edit/${eid}`);
    }
    ...
    const viewEvent = (row) => {
      tsMeetingListApp.currentId = row.eid;
      tsMeetingListApp.viewModalShowing = true
      gridOptions.showDetailsTitle = '查看';
    }

    /*TODO:删除事件*/
    const deleteEvent = (row) => {
      TsMeetingService.deleteTsMeeting(row.eid).then(() => {
