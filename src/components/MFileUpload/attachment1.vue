<template>
  ...
  <a-row v-else-if="operatorType==='view'">
    <div v-if="fileList.length ===0&&reFileList.length===0">
      <span style="color: blue;cursor: pointer">暂无资源包</span>
    </div>
  </a-row>
  <div>
  ...
  methods: {
    ...
    findFile(associateFormId, associateFormName) {
      if (associateFormId != null && associateFormName != null) {
        uploadAttachmentService.findFileOfResource(associateFormId, associateFormName).then((res) => {
          // console.log(res.data)
          this.reFileList = res.data;
        }).catch((error) => {
          VXETable.modal.message({content: `加载资源包文件出错，原因是：${error.message}`, status: 'error'});
        });
      }
    },
    //下载附件
    ...
    beforeUpload(file,fileList) {
      ...
      const maxSize = file.size / 1024 / 1024 <= 100; //小于100M
      // alert(file.size)

      // alert(fileList.length)
      // alert(this.fileList.length)
      // alert(this.reFileList.length)
